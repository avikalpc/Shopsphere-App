        ...
        {/* <div className='mt-12'> */}
          ...
          <div className='flex items-center mt-2 mb-2 md:flex md:flex-col'>
            ...
            <div className='flex flex-row items-center text-center ml-8 md:flex-col'>
              <button className='bg-[black] rounded-full text-white m-3 p-2 w-fit' onClick={() => dispatch(increase(id))}>+</button>
              <span className='flex items-center justify-center'>{amount}</span>
              <button className='bg-[black] rounded-full text-white m-3 p-2 w-fit' onClick={() => {
                if(amount === 1) {
                  dispatch(removeItem(id));
                  return;
                }
          ...
          <div className='flex sm:flex-col'>
            <Link to={`/product/${id}`} className='p-2 bg-[black] text-white rounded-md  hover:bg-black-500 mr-4 mt-4 mb-4'>Details</Link>
            <button onClick = {() => handleRemove(id)} className='p-2 bg-[black] text-white rounded-md mt-4 mb-4'>Remove</button>
          </div>
      </div>
    </>
  )
