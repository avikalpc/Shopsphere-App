  ...
  return (
    <div className='bg-white p-7 m-10 w-48 text-center rounded-md font-bold relative'>
      ...
      <div className="flex flex-col place-items-end inset-x-0 bottom-0 border-b-2">
        <Link to={`/product/${id}`} className='p-2 bg-[black] text-white rounded-md mt-4 w-full hover:bg-black-500'>Details</Link>
        <button onClick={() => handleCart({id, image, title, price})} className='p-2 bg-[black] text-white rounded-md mt-4 w-full hover:bg-black-500'>Add to Cart</button>
      </div>
    </div>
  )
}
